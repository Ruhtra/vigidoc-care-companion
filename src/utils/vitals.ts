...
export function getVitalStatus(key: string, value: number | null | undefined): VitalStatus {
  switch (key) {
    case 'heart_rate':
    case 'heartRate':
      return resolveHeartRate(value);
    case 'oxygen_saturation':
    case 'oxygenSaturation':
      return resolveO2Saturation(value);
    case 'temperature':
      return resolveTemperature(value);
    case 'systolic':
    case 'bloodPressure':
      return resolveSystolicPressure(value);
    case 'pain_level':
    case 'painLevel':
      return resolvePain(value);
    default:
      return resolveGeneric(value);
  }
}
